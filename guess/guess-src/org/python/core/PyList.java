...
public class PyList extends PySequence implements ClassDictInit, Collection
{
    ...
    public PyList(java.util.Collection collection)
    {
	length = collection.size();
	
	list = new PyObject[length];
...
//         }
	...
	Tracker.incrementLocation(1);
        for (int i=0; i<length-1; i++) {
            buf.append(((PyObject)list[i]).__str__().toString());
            buf.append(", ");
	    Tracker.incrementLocation(2);
        }
        if (length > 0)
            buf.append(((PyObject)list[length-1]).__str__().toString());
        buf.append("]");
        ts.exitRepr(this);

	String toRet = buf.toString();

	if (containsGraphElements()) {
    ...
    public String toString() {
        ...
        StringBuffer buf = new StringBuffer("[");
        for (int i=0; i<length-1; i++) {
            buf.append(((PyObject)list[i]).toString());
            buf.append(", ");
        }
        if (length > 0)
            buf.append(((PyObject)list[length-1]).toString());
        buf.append("]");
        ts.exitRepr(this);
	String toRet = buf.toString();
        return buf.toString();
    ...
    class PyListIterator implements Iterator {
