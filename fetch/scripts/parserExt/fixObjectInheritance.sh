...
python $FETCH/scripts/parserExt/fixObjectInheritance.py $DUMPDIR
...
echo "$OBJDECL"    >> $DUMPDIR/$2.classes
echo "$OBJMETHODS" >> $DUMPDIR/$2.methods
echo "$OBJMETHODS" >> $DUMPDIR/$2.methoddefs
echo "$OBJPKG"     >> $DUMPDIR/$2.packages
echo "$OBJFILE"    >> $DUMPDIR/$2.files
