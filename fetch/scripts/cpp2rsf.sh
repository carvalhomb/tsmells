...
then
	...
	$SN --batchmode --create  -D parser-ext="c++","*.[ch]pp *.cc *.hh *.h *.cxx QtTest" >> $LOG_FILE
fi
...
$ECHO $activityDescr >> $LOG_FILE
t1=`date '+%s'`
allFiles=`find . \( -name "Qt*" -o -name "*.cpp" -o -name "*.c" -o -name "*.cc" -o -name "*.cxx" -o -name "*.hpp" -o -name "*.h" -o -name "*.hh" -o -name "*.hxx" \)`
