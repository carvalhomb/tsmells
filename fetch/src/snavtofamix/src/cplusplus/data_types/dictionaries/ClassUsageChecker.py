...
class ClassUsageChecker(NamespaceContainmentChecker):
	##
	# Retrieve a list of ClassUsageReferences in the given file.
	##
	def getUsingDeclarationsInFile(self, fileName):
		usingDeclarations = []

		if self.namespaceDict.fileInDictionary(fileName):
			for reference in self.namespaceDict.getReferences(fileName):
				usingDeclarations.append(reference)
